...
# Define a mapping from class name to binary label
class_mapping = {
    'negative': 0,  # No Fracture
    'positive': 1   # Fracture Present
}
    ...
    for filename in os.listdir(label_dir):
        if filename.endswith('.txt'):
            ...
            if '_negative' in filename:
                class_label = class_mapping['negative']
            elif '_positive' in filename:
                class_label = class_mapping['positive']
            else:
                continue  # Skip any unexpected class
...
# Convert class labels to string for binary classification (if needed)
# This step is optional since class_mode is 'binary'
...
x = Dropout(0.5)(x)
predictions = Dense(1, activation='sigmoid')(x)  # Use 1 output for binary classification
...
cr = classification_report(y_true, y_pred_classes, target_names=['No Fracture', 'Fracture Present'])
